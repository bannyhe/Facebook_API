...
file_rev = open("nested_data_ps6.json", "r")
file_read = file_rev.read() 
file_diction = json.loads(file_read)
...
file_name = "python_diction_saved.json"
file_object = open(file_name, "w")
json_str = json.dumps(python_diction)
...
response = baseurl + airport
airport_response = requests.get(response, params = url_parameters)
...
def get_airport(string):
    new_url = baseurl + string
    airport_info = requests.get(new_url, params = url_parameters)
    return airport_info.json()
...
def extract_airport_data(string):
    dict = get_airport(string)
    return (dict['IATA'], dict['status']['reason'], dict['weather']['temp'], dict['weather']['meta']['updated'])
...
airport_tuples = []
for city in fav_airports:
    airport_tuples.append(extract_airport_data(city))
...
poss_airport_tuples = []
for city in possible_airports:
    try:
        poss_airport_tuples.append(extract_airport_data(city))
    except:
        print("Sorry, " + city + " didn't work.")
...
airport_stat = []
for title in fav_airports:
    airport_stat.append(extract_airport_data(title))

for stat in airport_stat:
    outfile.write("{}, {}, {}, {}\n".format(*stat))
outfile.close()
